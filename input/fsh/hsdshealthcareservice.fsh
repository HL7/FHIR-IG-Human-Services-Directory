...
* language  -> "	GAP in HSDS.  This FHIR element refers to the language in which the resource is written, not the language spoken at the location in which this service is delivered. This is found in HealthcareService.communication, mapped below. This optional element can be ignored."
* text  -> " GAP in HSDS.	FHIR HealthcareService.text is datatype 'Narrative' and HSDS service.description does not approproiately map to HealthcareService.text and this optional element will be ignored. "
...
* extension[delivery-method]  -> "	Although 'delivery method for services' is a GAP in HSDS, (the HSDS location and service_at_location tables provide details about services at the locations in which they are delivered. The HSDS location table 'provides details of the locations where organizations operate. Locations may be virtual, and one organization may have many locations'. It's unclear whether the type (virtual vs physical) is found in service_at_location.description or location.description. A new attribute field could be added to the HSDS service_at_location table to reflect this concept, but until then, this Must Support, but optional extension will be ignored.  "
...
* identifier.id  -> "	GAP in HSDS. Ignore. "
// * Slices for extension  -> "	GAP in HSDS		"
...
* communication  -> "	HealthcareService.communication points to a list of codeableConcepts bound to the Common Languages value set [http://hl7.org/fhir/ValueSet/languages] but the HSDS language table does not contain coded values as language.language fied description implies (Languages, other than English, in which the service is delivered. Languages are listed as ISO639-1 codes.) So the mapping will be done without using codes as follows: communication[0].text = language.language (Languages, other than English, in which the service is delivered. Languages are listed as ISO639-1 codes.) Based on documentation related to HSDS 3.0 updates, any table.field names that are identical (e.g., language.language) may be changed in the upcoming release.	"
...
* availableTime.modifierExtension  -> "  May want to create a new modifier extension - ScheduleValidDates to reflect schedule.valid_from & schedule.valid_to dates. 	"
